...
Box.test(shampoo.m1$residuals, lag = 20, type = 'Ljung-Bo', fitdf = 4)
...
# two candidate models: ARIMA(1,1,1) with drift, ARIMA(0,1,1) with drift
m1 <- Arima(window(shampoo, end = c(2003, 6)), order = c(1, 1, 1), 
            include.drift = TRUE)
m2 <- Arima(window(shampoo, end = c(2003, 6)), order = c(0, 1, 1),
            include.drift = TRUE)
